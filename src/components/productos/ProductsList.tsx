    ...
    return <Card>
        <CardContent className="p-8 text-center">
          <div className="text-gray-500">
            ...
            <p className="text-sm">
              {showOwnerActions ? "Aún no has publicado ningún producto. ¡Publica tu primer producto para comenzar!" : "No hay productos disponibles en este momento. ¡Vuelve pronto!"}
            </p>
          </div>
        </CardContent>
      </Card>;
  ...
  return <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
      {productos.map(producto => <Card key={producto.id} className="hover:shadow-lg transition-shadow">
          <CardHeader>
            ...
            <div className="flex justify-between items-start">
              ...
              <div className="flex flex-col gap-1">
                {showOwnerActions && <Badge variant={producto.status === 'aprobado' ? 'default' : producto.status === 'rechazado' ? 'destructive' : producto.status === 'suspendido' ? 'secondary' : 'outline'}>
                    {producto.status === 'aprobado' ? 'Aprobado' : producto.status === 'rechazado' ? 'Rechazado' : producto.status === 'suspendido' ? 'Suspendido' : 'Pendiente'}
                  </Badge>}
            ...
            {/* Precio destacado */}
            {producto.precio_unidad && <div className="mt-2 p-3 bg-green-50 border border-green-200 rounded-lg">
                <div className="text-xl font-bold text-green-800">
                  $ {producto.precio_unidad.toLocaleString('es-CO')} COP
                </div>
                {producto.incluye_domicilio && producto.costo_domicilio > 0 && <div className="text-sm text-green-600 mt-1">
          ...
          <CardContent className="space-y-4">
            ...
            {producto.incluye_domicilio && producto.direccion_vendedor && <div className="text-sm text-blue-600 bg-blue-50 px-2 py-1 rounded">
                <strong>Dirección:</strong> {producto.direccion_vendedor}
              </div>}

            {/* Imagen de previsualización */}
            {producto.imagenes && producto.imagenes.length > 0 ? <div className="mb-4 relative cursor-pointer" onClick={() => handleViewImages(producto)}>
                <img src={producto.imagenes[0]} alt={producto.nombre} className="w-full h-40 object-cover rounded-lg hover:opacity-90 transition-opacity" />
                {producto.imagenes.length > 1 && <div className="absolute bottom-2 right-2 bg-black bg-opacity-75 text-white px-2 py-1 rounded text-xs">
              ...
              </div> : <div className="mb-4 h-40 bg-gray-100 rounded-lg flex items-center justify-center">
            ...
            <div className="flex items-center text-gray-500 text-xs">
              ...
              <span>
                Publicado: {format(new Date(producto.created_at), 'PP', {
              locale: es
            })}
              </span>
            </div>

            {producto.imagenes && producto.imagenes.length > 0 && <div className="text-xs text-gray-500">
                {producto.imagenes.length} imagen(es) disponible(s)
              </div>}

            {showOwnerActions && producto.user_id === user?.id && producto.status !== 'rechazado' && producto.status !== 'suspendido' && <div className="flex gap-2 pt-2">
                <Button variant="outline" size="sm" onClick={() => handleToggleDisponible(producto)} disabled={loadingIds.has(producto.id) || producto.status !== 'aprobado'} className="flex-1">
                  <Edit2 className="w-4 h-4 mr-1" />
                  {producto.disponible ? 'Marcar no disponible' : 'Marcar disponible'}
                </Button>
                <Button variant="destructive" size="sm" onClick={() => handleDelete(producto.id)} disabled={loadingIds.has(producto.id)}>
                  <Trash2 className="w-4 h-4" />
                </Button>
              </div>}
            
            {showOwnerActions && producto.user_id === user?.id && (producto.status === 'rechazado' || producto.status === 'suspendido') && <div className="text-center py-2">
